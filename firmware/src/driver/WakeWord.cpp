...
void wakeword_init(){
  ...
  Serial.printf("Wakeword loaded count: %d\n", loaded_count);
}

int wakeword_regist() // 検出した音声を再生しMFCCを登録及びファイルに保存します
{
  //bool ret =false;
  int ret = WAKEWORD_REGIST_NO_RESULT;
  // M5.Speaker.end();
  // M5.Mic.begin();
  auto* data = rxMic();
  // M5.Mic.end();
...
//  if (length <= 100) { return ret; }
  ...
  if (mfcc_tmp)
  {
    ...
    if (!stored) {
      delete mfcc_tmp;
      ret = WAKEWORD_REGIST_NO_SLOT;
      Serial.println("Wakeword register failed: no empty slot");
    }
  }
  vadEngine.reset();
//  mode = 0;
...
int wakeword_compare()  
{
      ...
      || (state >= simplevox::VadState::Speech && mfccFrameNum <= mfccFrameCount))
  {
    ...
    if (!feature)
    {
      Serial.println("Wakeword: failed to create MFCC feature");
      raw_reset();
      mfccFrameCount = 0;
      vadEngine.reset();
      return -1;
